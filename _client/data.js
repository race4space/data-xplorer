class cls_data {
  ...
  fn_execute(){
    this.fn_build_interface_client();
  ...
  fn_build_interface_design(){
    this.str_data_action="interface-design";
    return this.fn_open();
  }
  fn_reset_nav(){
  ...
  fn_write_nav_control(obj_ini){
    var str_html, s;
    s='';
    s+='<form frm-srh nav-face class="form-inline mt-2" id="'+obj_ini.form_search_id+'" style="display:none">';
    ...
    s+='<nav-records style="display:none">';
    if(obj_ini.bln_nav){
